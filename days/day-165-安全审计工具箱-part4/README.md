...
### 思考 1：为什么报告层不能用 `datetime.now()`？

请顺着链条推：不确定的时间戳 → 报告每吹不同 → `git diff` 失效 → …… →
最后会影响哪一个业务能力？（提示：从“回归对比”推到“审计结论能不能被复核”。）

> 参考方向：报告一旦不可复现，别人就无法验证你的结论，
> 审计就从“可复核的证据”退化成“一张截图”。

### 思考 2：如果两个不同的问题碰巧有相同的 `(target, rule_id, severity)`，会怎样？

本课的 id 只由这三个字段组成。请找出一个真实场景能造成这种碰撞，
并给出一个修法（同时**保住**“改文案不换 id”这个优点）。

> 参考方向：同一 URL 上同一规则命中两次（例如两个不同参数都触发了 XSS）。
> 修法：把“定位到具体位置”的信息（参数名/行号）加进 id，
> 但保持 `title`/`reason` **不**入哈希。

### 思考 3：HTML 渲染为什么必须 `html.escape()`？举一个具体攻击路径

本题是安全的重点。请完整写出：
（a）被审计目标返回了什么；（b）报告怎么把它带进去；
（c）谁会被成功攻击；（d）多了一层什么防护就能阻断。

> 参考方向：见 3.6 节。关键认识是：**审计工具处理的是不可信输入**。
> 一个功能正常的“报告预览”界面就足以把存储型 XSS 送到审计员浏览器里。

### 思考 4：为什么退出码 4（覆盖缺口）不能和 0（通过）合并？

请从“行动项”角度回答，而不是从“返回值”角度。
如果一个 CI 流水线把 4 当成 0 处理，长期会发生什么？

> 参考方向：把“没测到”当成“没问题”，于是“什么都测不到”的配置
> 会看起来永远绿灯。这是审计工具最危险的失败模式。

### 思考 5：门禁用原始 severity，排序用风险分——为什么不统一成一套？

如果你强行把两者统一（比如门禁也乘置信度系数），会出现什么后果？
请举出一个具体的“不该放过但会被放过”的例子。

> 参考方向：见 6.3 节。低置信度 high 在风险分里是 7（比 medium 还低），
> 如果门禁用风险分，那么“证据不完美的高危”就会被合法放过。
> 门禁与排序回答的是两个不同问题：**能不能放过 vs 先看哪个**。
...
如果现在有人问你“你这个工具能保证我的系统安全吗？”，
诚实的回答包含三件事：**不能保证**（只覆盖测过的范围）、
**覆盖缺口在哪**（主动说出盲区）、**结论有边界**（未发现 ≠ 没有）。

> 一个声称“保证安全”的安全工具，要么在说谎，要么在把责任转移给使用者。
> **一个诚实的工具的标配，是把自己的边界说清楚。**
> 这句话是这四天里比任何一行代码都重要的东西。

---

**下一天**：Day 166 — 系统监控（psutil）。
从“审计别人的系统”转向“保护自己的系统”：
CPU / 内存 / 磁盘 / 网络监控、进程管理、系统信息采集，以及一个可用的监控脚本。
