...
d = threading.Thread(target=daemon_worker, daemon=True)   # 守护线程
d.start()
n = threading.Thread(target=normal_worker)                # 非守护线程
n.start()

print("主线程到这里就返回了")   # 主线程结束，但进程不会退出
```

运行顺序推演：

1. `d.start()` 后，`daemon_worker` 进入 `while True` 死循环，不停打印。
2. `n.start()` 后，`normal_worker` 开始跑 3 秒的任务。
3. 主线程执行到最后一行 `print` 就**返回了**——但进程没有退出，因为普通线程 `n` 还在运行。
4. 注意这一步：守护线程 `d` **没有停**，它继续打印。主线程"先走了"并没有把它带走。
5. 直到 `n` 跑满 3 次，**所有非守护线程都结束**，Python 进程才终止。
6. 进程终止的一瞬间，守护线程 `d` 才被**直接终结**——它的死循环永远不会自己结束，是进程退出把它带走的。

结论：

- 守护线程不是"主线程一退出就自杀"，而是**等其他非守护线程全部结束后，进程退出时把它终止**。
- 决定退出时机的关键点是**最后一个非守护线程**，不是主线程。
- 守护线程适合后台监控、定时心跳、日志刷写这类丢了也无所谓的任务。
- 需要保证完成的任务（写文件、提交数据库）要用普通线程并 `join()` 等待。

### 3.4 daemon 的设计原理

**进程什么时候退出？** 当所有非守护线程都结束后，进程才会退出。主线程本身就是非守护线程。

**守护线程扮演什么角色？** 它是被设计成"进程退出时可以被丢弃"的后台任务。进程决定退出时，**不会等待守护线程**，直接把它结束。

**这带来什么后果？** 守护线程可能在任意时刻、工作做到一半时被终止，连 `finally` 都不会执行。所以不要在守护线程里放"退出前必须完成"的收尾逻辑——这类逻辑应该用普通线程加 `join()` 来保证。

机制图：

```
时刻 A：主线程已返回，普通线程 n 还在跑
┌─────────┐   ┌─────────────┐   ┌──────────────┐
│ 主线程(已返回) │   │ 普通线程 n   │   │ 守护线程 d   │
│          │   │ 仍在运行     │   │ 仍在运行      │
└─────────┘   └─────────────┘   └──────────────┘
               ↑ 非守护没结束 → 进程还活着 → d 也活着

时刻 B：n 跑完最后一圈，所有非守护线程结束
┌──────────────────┐   ┌──────────────────────┐
│ 非守护线程全部结束 │   │ 进程终止             │
└──────────────────┘──▶│ 不等待 d，d 被直接结束 │
                        │ （不清理，finally 不执行）│
...
一句话理解：守护线程像扫地阿姨，普通线程像留在工位的员工。只要还有员工在加班，阿姨就继续扫；最后一个员工离开，楼断电，阿姨也被强制清场——关键开关在"最后一个非守护线程"，而不是"老板（主线程）先走"。
